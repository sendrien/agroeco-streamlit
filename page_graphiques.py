...
def radar_plot(radar_df, labels, categories_labels):
    ...
    fig.update_layout(
        title_text="",  # évite "undefined"
        polar=dict(
            radialaxis=dict(visible=True, range=[0, 3.5], showticklabels=True, tickfont_size=13),
            angularaxis=dict(tickfont=dict(size=15)),
        ),
        showlegend=True,
        legend=dict(x=1.0, y=1.0, xanchor="left", font=dict(size=12)),
        margin=dict(l=30, r=120, t=100, b=20),  # top ↑ pour laisser la place au bouton
        paper_bgcolor="#fff", plot_bgcolor="#fff",
        updatemenus=[{
            "type": "buttons",
            "showactive": False,
            "y": 1.18,   # sous le titre, au-dessus de la zone de tracé
            "x": 0.0,
            "xanchor": "left",
            "yanchor": "top",
            "buttons": [{
...
def bar_chart_anim(radar_df, dim_labels, cat_labels, cat_idx=0):
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            ...
            height=430,
            margin=dict(l=130, r=60, t=100, b=60),  # top ↑ pour le bouton
            plot_bgcolor="#fff", paper_bgcolor="#fff", showlegend=False,
            updatemenus=[{
                "type": "buttons",
                "showactive": False,
                "y": 1.16,
                "x": 0.0,           # plus de x négatif -> pas de chevauchement
                "xanchor": "left",
                "yanchor": "top",
                "buttons": [{
                    "label": "Dessiner",
...
def bar_effectifs_stacked_anim(eff_df):
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            ...
            barmode='stack',
            height=500,
            margin=dict(l=170, r=40, t=100, b=70),  # top ↑ pour le bouton ; bottom ↑ pour la légende
            xaxis=dict(title='Nombre de répondants', dtick=50, range=[0, eff_df.values.sum(axis=1).max() * 1.08]),
...
def pie_poids_relatif_anim(categories, poids_relatif):
    ...
    fig.update_layout(
        title_text="",
        margin=dict(l=20, r=20, t=90, b=20),  # top ↑ pour le bouton
        height=500,
        showlegend=True,
